...
def get_stock_info(filename, side):
    ...
    if img is None:  # the imread() function returns a NoneType object on error.
        ...
        for detection in detections:
            left_x = detection["box_points"][0]
            left_y = detection["box_points"][1]
            right_x = detection["box_points"][2]
            right_y = detection["box_points"][3]

            middle_x = (left_x + right_x) / 2
            middle_y = (left_y + right_y) / 2

            # To decide which side of the cooler
            if side == '3' and (middle_y < 645 or (middle_x < 1457 and middle_y > 1365) or (middle_x > 1457 and middle_y > 1657)):  # left
                continue
            elif side == '1' and (middle_y < 457 or (middle_x < 937 and middle_y > 1160) or (middle_x > 937 and middle_y > 1440)):  # right
                continue
...
while True:
    ...
    if len(file_names) <= 1:
        ...
        for i in range(len(file_names_grouped_by_mac)):
            ...
            is_shelf_available = [False, True, False]
            ...
            for j in range(len(file_names_grouped_by_shelf)):
                ...
                get_stock_info(file_name, shelf_number)
            ...
            if False in is_shelf_available:  # one of the shelves is not available
                ...
                print(
                    'The shelves ' + str([x + 1 for x in indexes]) + ' are not available for cooler ' + str(mac) + '.')
                logging.error(
                    'The shelves ' + str([x + 1 for x in indexes]) + ' are not available for cooler ' + str(mac) + '.')
            # TODO: handle shelf not available situation
                ...
                try:
                    ...
                    if response:
                        ...
                        logging.error('Response is not OK.' + r.content)
                        print('Response is not OK.' + r.content)
                except:
